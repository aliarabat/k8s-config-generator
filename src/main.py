...
class k8s_config:
    def __init__(self):
        ...
        self.namespace_start = args.namespace_start
        self.group_count = args.group_count
    ...
    def generate(self):
        ...
        for i in range(1, int(self.group_count) + 1):
            for j in range(int(self.namespace_start), int(self.namespace_count) + 1):
                team_name = f"grp0{i}eq{j}"
    ...
    def generate_files(self):
        ...
        # self.team_key = base64.b64encode(open(self.key_file, "r").read().encode(
        #     "ascii")).decode('ascii').replace('\n', '')

        # self.team_cert = os.popen(f"cat {self.crt_file} | base64 | tr -d '\n'").read()

        # self.team_cert = base64.b64encode(open(self.crt_file, "r").read().encode(
        #     "ascii")).decode('ascii').replace('\n', '')
